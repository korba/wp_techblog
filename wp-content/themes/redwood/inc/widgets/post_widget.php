...
class solopine_latest_news_widget extends WP_Widget {
	...
	function solopine_latest_news_widget() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'solopine_latest_news_widget', 'description' => __('A widget that displays your latest posts from all categories or a certain', 'redwood') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'solopine_latest_news_widget' );

		/* Create the widget. */
		parent::__construct( 'solopine_latest_news_widget', __('Redwood: Latest Posts', 'redwood'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => __('Latest Posts', 'redwood'), 'number' => 5, 'categories' => '');
		$instance = wp_parse_args( (array) $instance, $defaults ); ?>

		<!-- Widget Title: Text Input -->
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e('Title:', 'redwood'); ?></label>
		...
		<!-- Number of posts -->
		<p>
			<label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e('Number of posts to show:', 'redwood'); ?></label>
